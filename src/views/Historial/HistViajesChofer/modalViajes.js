...
const ModalViajes = (props) => {
    ...
    const traerDatos = async () => {
        ...
        data.append('idChofer', dataEnviar.idChofer);
        const histviajes = await axios.post(backend + "getHistorialViajesXchofer", data);
        const datos = histviajes.data.historialViajesChofer;

        if (datos) {
            datos.forEach((element, index) => {
                const fechaViaje = new Date(Date.parse(element.fechaViaje));
                datos[index].fechaViaje = moment(fechaViaje).format('DD/MM/YYYY - HH:mm');
            });
            setDatam(histviajes.data.historialViajesChofer);
            setLoading(false);
        } else {
            setDatam(histviajes.data.historialViajesChofer);
            setLoading(false);
